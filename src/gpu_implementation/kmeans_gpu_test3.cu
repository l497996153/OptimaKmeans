        ...
        if (cluster_id >= 0 && cluster_id < K) {
            ...
            for (int d = 0; d < dimensions; d++) {
                ...
                for (int src = 0; src < 32; src++) {
                    double other = __shfl_sync(group, v, src);
                    if (lane == leader && ((group >> src) & 1)) {
                        group_sum += other;
                    }
                }

                if (lane == leader) {
                    atomicAdd(&shared_sums[cluster_id * dimensions + d], group_sum);
                }
            }
                   ...
                   int *host_clusters, int *finished_iterations, int threads_per_block)
{
    ...
    memset(host_clusters, -1, (size_t)num_points * sizeof(int));
    ...
    int reduce_items = k * dimension + k;
    int reduce_blocks = (reduce_items + reduce_threads - 1) / reduce_threads;
    if (reduce_blocks < 1) {
        reduce_blocks = 1;
    }
    if (reduce_blocks > 256) {
        reduce_blocks = 256;
    }
